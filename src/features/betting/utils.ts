  ...
  const filtered = betResults.filter(
    (result) =>
      (dateFilter === "all"
        ? true
        : dateFilter === "playoffs"
          ? result.game_date >= playoffStartDate.toISOString().split("T")[0]
          : result.game_date >= dateRange.toISOString().split("T")[0]) &&
      (thresholdFilter === "all"
        ? true
        : thresholdFilter === 6
          ? result.threshold <= 3
          : result.threshold === thresholdFilter) &&
      (typeFilter === "all"
        ? true
        : typeFilter === "over"
          ? result.bet_type !== "under"
          : result.bet_type === typeFilter),
  )
  return filtered
}

export function computeCumulativeProfit(
  betResults: BetResult[],
): { game_date: string; cum_profit: number }[] {
  if (betResults.length === 0) return []
