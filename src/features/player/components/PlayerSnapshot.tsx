  ...
  return (
    <div className="p-4 ml-15 bg-black-300 rounded-lg">
      ...
      <div className="space-y-1 text-sm">
        ...
        <p>
          Goals: {snapshot.goalsTotal} ({snapshot.goalsPerGame.toFixed(2)} p/g)
        </p>
        <p>
          Assists: {snapshot.assistsTotal} ({snapshot.assistsPerGame.toFixed(2)}{" "}
          p/g)
        </p>
        <p>
          Points: {snapshot.pointsTotal} ({snapshot.pointsPerGame.toFixed(2)}{" "}
          p/g)
        </p>
        <p>
          SOG: {snapshot.shotsTotal} ({snapshot.shotsPerGame.toFixed(2)} p/g)
        </p>
        <p>
          SATT: {snapshot.shotAttemptsTotal} (
          {snapshot.shotAttemptsPerGame.toFixed(2)} p/g)
        </p>
      </div>
    </div>
  )
